#include <stdio.h>
#include "libgit2/include/git2.h"

// doh ~stub
void get_master_hex(char *hex) {
    FILE *f = fopen(".git/refs/heads/master", "r");
    fgets(hex, 41, f);
    fclose(f);
}

int main (int argc, char** argv) {
    if (strcmp(argv[1], "rev-list") != 0) {
        err(0, "Not implemented!\n");
        exit(1);
    }

    if (argc != 3) {
        err(0, "Usage: git2 rev-list <commit>\n");
        exit(1);
    }

    char *hex = (char*)malloc(41*sizeof(char));
    get_master_hex(hex); // do not look what's inside

    int e;
    // Start rev-list code //
    git_repository *repo;
    if (e = git_repository_open(&repo, ".git") != 0)
        err(1, "Failed git repo open: %d" , e);

    git_oid id;
    git_revwalk *walk;
    git_commit *head, *commit;

    git_oid_mkstr(&id, hex);
    //printf("Raw 20 bytes: [%s]\n", (&id)->id);

    git_odb *odb; // fetch the object!
    odb = git_repository_database(repo);

    if (e = git_commit_lookup(&head, repo, &id) != 0)
        err(1, "Error in git commit lookup: %d\n");
    if (e = git_revwalk_new(&walk, repo))
        err(1, "Fail walker: %d\n", e);
    //git_revwalk_sorting(walk, GIT_SORT_TOPOLOGICAL | GIT_SORT_REVERSE);
    if (e = git_revwalk_push(walk, &id))
        err(1, "Failed revwalk push: %d\n", e);

    int ret = git_revwalk_next(&id, walk);
    char *hex2 = (char*) malloc(40*sizeof(char));

    git_oid_fmt(hex2, &id);
    
    while (git_revwalk_next(&id, walk) == GIT_SUCCESS) {
        git_oid_fmt(hex2, &id);
        printf("%s\n", hex2);
    }
    return 0;
}
